 ...
 *
 * 2.要求：向Set中添加的数据，其所在的类一定要重写hasCode()以及equals().
 *   要求：重写的hashCode()和equals()尽可能保持一致性：相同的必须具有相同的散列码
...
public class SetTest {
    ...
    二、添加元素的过程：以HashSet为例：
        我们向HashSet添加元素a,首先调用元素a所在类的hashCode()方法，去计算元素a的哈希值,
        ...
        数组此位置上是否以及有元素:
            ...
            如果此位置上有其他元素b（或以链表形式存在的多个元素），则比较元素a与元素b的hash值：
                ...
                如果hash值相同，进而需要调用元素a所在类的equals()方法：
                        ...
                        equals()返回false,元素a添加成功 --->情况2

        对于添加成功的--->情况2和情况3而言：元素a 与 已经存在指定索引位置上数据以链表的方式存储。
