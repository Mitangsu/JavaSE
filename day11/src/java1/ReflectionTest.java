...
public class ReflectionTest {
    ...
    public void test(){
        ...
        //在Person类外部，不可以通过Person类的对象调用其内部其私有的结构。
    ...
    //疑问1：通过直接new的方式反射的方式都可以调用公共的结构，开发中到底用哪个？
    //建议：直接new的方式
    //什么时候回使用：反射的方式。反射的特征：动态性
    ...
    1.类的加载过程：
    程序经过javac.exe命令以后,回生成一个或多个字节码文件(.class结尾),接着我们使用
    java.exe命令对某一个字节码文件进行解释运行.相当于某个字节码文件加载到内存中.
